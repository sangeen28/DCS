...
class DeviceCapability:
    ...
    def sample(device_type: str, rng: np.random.Generator) -> "DeviceCapability":
        ...
        if dt == "smartphone":
            ...
            raise ValueError(f"Unknown device type: {dt}")
    ...
    def latency_components(
        ...
        # Demo proxy: compute scales with steps and data; comm scales with model size / bandwidth
        comp = (local_steps * max(1, data_size) * 1e-6) / max(1e-6, self.cpu_ghz)
        comm = (2.0 * model_bytes * 8.0) / max(1e-6, self.bandwidth_mbps * 1e6)
        overhead = float(np.random.exponential(0.20))
        return float(comp), float(comm), float(overhead)
...
class IoTDevice:
    def __init__(
        ...
        train_dataset: Dataset,
        batch_size: int,
        seed: int = 42,  # accepted for reproducibility / compatibility
    ):
        _ = seed  # seed is controlled globally via set_seed; kept for API stability
    ...
    @property
    def base_subset(self) -> Subset:
        # legacy alias (some code uses base_subset)
        return self.subset
    ...
    def local_train(
        ...
        steps_proxy = int(local_epochs * math.ceil(len(self.subset) / max(1, self.batch_size)))
        comp_s, comm_s, over_s = self.cap.latency_components(
            model_num_bytes(global_model), steps_proxy, len(self.subset)
        )
        latency_total = float(train_wall + comp_s + comm_s + over_s)
...
class CloudServer:
    ...
    def evaluate_accuracy(self, dataset: Dataset, device: torch.device, batch_size: int = 256) -> float:
        return float(eval_accuracy(self.global_model, dataset, device=device, batch_size=batch_size))

    def apply_delta(self, delta_vec: np.ndarray, device: torch.device) -> None:
        if delta_vec is None:
            return
        gvec = parameters_to_vector(self.global_model.parameters()).detach().cpu().numpy().astype(np.float32)
        new = gvec + delta_vec.astype(np.float32)
        vector_to_parameters(torch.from_numpy(new).to(device), self.global_model.parameters())
        self.round += 1
